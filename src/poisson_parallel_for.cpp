...
int main() {
	...
	// Parámetros de la simulación (se calculan aquí ya que no están en initialize_grid)
	double dx = 1.0 / (M + 1);
	double dy = 1.0 / (N + 1);
	double dx2 = dx * dx;
	double dy2 = dy * dy;
...
#pragma omp parallel for reduction(max:max_delta)
		// CORRECTO: Los límites del bucle son de 1 a M (inclusive), como en la versión serial
		for (int i = 1; i <= M; ++i) {
			for (int j = 1; j <= N; ++j) {
				T_new[i][j] = factor * (
						(T[i-1][j] + T[i+1][j]) / dx2 +
		...
		// El bucle de copia también debe tener los límites correctos
