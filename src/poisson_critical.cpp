...
#pragma omp parallel for reduction(max:max_delta)
	    ...
	    // --- COPY LOOP & CRITICAL SECTION INCREMENT ---
...
#pragma omp for
		    ...
		    // --- CRITICAL SECTION DEMONSTRATION as per Activity 6 ---
		    // We use 'single' to ensure only one thread executes this block.
		    // We use 'critical' to protect the increment of the shared 'iterations' variable.
#pragma omp single
		    {
#pragma omp critical
			    {
